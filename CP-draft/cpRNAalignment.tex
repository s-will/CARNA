...
dendencies specialized to proteins~\cite{caprara02:_struc}.
...
essentially the same problem. On unlmited input, Lara scores only a
...
Therefore, $\operatorname{ub}(i,i')$ is computed in $O(kk')$ time,
where $k$ and $k'$ are the respective number of base pairs incident to
$i$ and $i'$. For crossing dependencies $k=k'=1$ and for many other
applications $k$ and $k'$ can be constantly
...
%% Independent sub problems can be solved independently (Dechter), supported by lower bounds due to propagator

%% special case: for independent subproblems without any arcs, trace yields best
%% solution and fixing variables to best solution does not remove the
%% COP solution
...
%
  \end{tabular}
  \caption{Rfam Riboswitches. For 100 instances from each Rfam family annotated as riboswitch and confirmed by literature: Medians of average sequence length, average number of base pairs, and run-time, maximal memory requirement and percentage of instances not solved to optimality within a given time limit of 1 min.}
  \label{tab:riboswitches}
\end{table}

\section{Discussion}

We presented a propagator for the problem of weighted string matching with arbitrary dependencies which is also known as the alignment problem for arc annotated sequences with unlimited structure.   
Whereas the problem itself is MAX-SNP-hard, our propagator relies on a relaxation of the problem that makes it tractable in a constraint programming framework. We also propose a search strategy that optimizes the benefit of the propagator and show how to decompose the weighted string matching problem into independent subproblems during search. 

To evaluate the applicablilty of our method in practice, we apply it to the alignment of RNA structures. While all previous approaches in this area are limited to score at most crossing subsets of the input structures, our approach is able to align unlimited structures. This is useful, for example, to align Riboswitches and allows to consider all their potential structural conformations simultaneously.
