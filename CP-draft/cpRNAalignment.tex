...
detecting independent subproblems and optimizing these subproblems
independently of each other~\cite{Dechter:and_or_bab:AI}. In general,
this introduces overhead for detecting dependencies and even interferes
with Branch-and-Bound when subproblems cannot be bounded
well. However, the string matching problem suggests a special form of
decomposition along the string order, where our propagator provides
upper bounds for the partial problems.
...
Notably, the dependencies due to an arc
$(i,j)$ can be resolved if one of its ends is matched, i.e. $\forall
A\in D(\cA): (i,i')\in A$ (or $\forall A\in D(\cA): (j,j')\in A$) by
moving the weight of each arc match $(i,j;i',j')$ to the match weight
of the other end $(j,j')$ (or $(i,i')$ respectively).
%% Gecode: no easy/efficient way to test for external dependency => implement only internal

In the RNA alignment problem, all constraint dependencies are known to
the propagator, which allows for testing independence within the
propagator.\footnote{This is important for our implementation, because
  general dependency checking for variables is expensive in Gecode.}
To avoid overhead, we apply the decomposition only if an independent
subproblem for $\var{MD}_{i},\dots,\var{MD}_{j}$,
...
subproblem are resolved. Then, the problem reduces to maximum
weighted string matching without dependencies and the traceback
alignment represents an optimal solution.  Assigning the traceback of
the subproblem to $\var{MD}_{i},\dots,\var{MD}_{j}$,
...
seconds. In all but one of the omitted cases, Carna solves the problem
without backtracking. In terms of performance, with the single
exception of tmRNA, both programs are on a par for the simpler class
...
\begin{table}
  ...
  \begin{tabular}{c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}|@{\hspace{6pt}}c@{\hspace{6pt}}}
    ...
    RNaseP\_arch     & 303 & 367 & 0.46  & 1.4  & 66 & 1 & 66 & 66 \\
    RNaseP\_bact\_b  & 408 & 401 & 3.0  & 2.3  &416 &23 &1282&1304\\
    RNaseP\_nuc      & 317 & 346 & 0.07 & 2.9  & 29 & 2 & 28 & 29 \\
    %RydC             &  64 &  66 & 0.01 & 0.0  & 0  & 1 & 0  & 0  \\
    Telomerase-vert  & 448 & 451 & 0.47 & 2.3  &181 & 7 &175 &181 \\
    tmRNA            & 384 & 367 & 63   & 3.7  &478 &28 &18937&18964\\
...
%
  ...
  \caption{%
    ...
    length, average number of base pairs, and run-time, maximal memory
...
the problem itself is MAX-SNP-hard, our propagator relies on a
relaxation of the problem that makes it tractable in a constraint
programming framework. We also propose a search strategy that
optimizes the benefit of the propagator and show how to decompose the
weighted string matching problem into independent subproblems during
search.
...
is useful, for example, to align Riboswitches and allows to consider
all their potential structural conformations simultaneously.
