...
effective than dynamic programming based approaches. 
\mm{is Lara not also able to align unlimited instead of crossing structures?} 
The success of
...
\end{align*}
\mm{
%I changed the order of the arguments from $\tau(i,i',j,j')$ to
%$\tau(i,j,i',j')$ etc to be consistent to previous occurences. 
Should we write $(i,i')\in A_m$ instead of $(i,i')\in A$? But then we should probably also write $A=A_m\cup A_g\in\cA(D)$. I am not sure.}.
...
%\mm{reordered  arguments of $\tau$, see earlier comment} OK
over alignments in
