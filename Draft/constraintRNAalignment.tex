...
\begin{abstract}
  ...
  structures.
  \mm{I like this better than "class of structures with unlimited
  complexity" since grammatically one can then also think that the
   approach has unlimited complexity}
...
parameterized complexity, were the parameter captures the complexity
of the
structures~\cite{Evans:finding:CPM99,Moehl:Will:Backofen:CPM2008}.
\mm{I changed the sentence, since I think parameterized complexity
 only works for alignment with fixed structures and not for
 prediction (where the structures are not fix). Maybe the additional
 evans reference is not necessary, if it takes too much space.}
...
effective than dynamic programming based approaches. \mm{is Lara not also able to align unlimited instead of crossing structures?}The success of
...
\mm{maybe we should insert here some words how this is related to the Lara model?}
...
\begin{align*}
  \operatorname{ubound}_D(i,i') := & \max_{A\in\cA(D)} \sum_{\substack{(i,j)\in P_a,\\(i',j')\in P_b,\\(i,i')\in A,\\(j,j')\in A}} \tau(i,j,i',j')
  & + \max_{A\in\cA(D)} \sum_{\substack{(j,i)\in P_a,\\(j',i')\in P_b,\\(i,i')\in A,\\(j,j')\in A}} \tau(j,i,j',i').
\end{align*}
\mm{I changed the order of the arguments from $\tau(i,i',j,j')$ to
  $\tau(i,j,i',j')$ etc to be consistent to previous occurences. Should we write $(i,i')\in A_m$ instead of $(i,i')\in A$? But then we should probably also write $A=A_m\cup A_g\in\cA(D)$. I am not sure.}.
...
maximization of $\sum_{\substack{(i,j)\in P_a,(i',j')\in
    P_b,\\(i,i')\in A,(j,j')\in A}} \tau(i,j,i',j')$ \mm{reordered
  arguments of $\tau$, see earlier comment} over alignments in
$\cA(D)$. A single match $(j,j')$ can occur in an alignment in
...
solved efficiently by dynamic programming.\mm{can we add a little bit
  more detail here?It this essentially, that we only want to consider
  sets of $(j,j')$'s that can be aligned simultaneously?} Thus,
